...
export default class StyleValidator {
  ...
  validate(style, componentName) {
    ...
    for (const propNameCamelCase of Object.keys(style)) {
      const propName = propNameCamelCase.replace(
        capsRe,
      ...
      if (!supportInfo) {
        if (this.config.strict) {
          return new Error(
            `Unknown style property \`${propName}\` supplied to \`${componentName}\`.`
          );
        }
      } else {
        const unsupported = [];
        ...
        if (this.config.warn) {
          ...
          for (const [msg, platforms] of messages) {
            console.warn(
              `Warning: Style property \`${propName}\` supplied to \`${componentName}\`, in ${platforms.join(
                ", "
              )}: ${msg.toLowerCase()}`
